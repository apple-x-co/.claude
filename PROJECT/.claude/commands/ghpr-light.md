...
PR 作成後、以下の操作を提案（簡潔に）:
- Draft から Ready への変更: `gh pr ready`
...
- **デフォルトで Draft PR を作成**
- **既存の PR との重複をチェック**
- **差分コミットの情報のみを使用**（Context や記憶は使用禁止）
...
✅ **使用してよい情報源**:
- `git log origin/<base-branch>..HEAD` の出力
...
**確認方法**:
生成した PR 内容の全ての情報が、ステップ4で取得したコマンドの出力から確認できることを確認してください。
